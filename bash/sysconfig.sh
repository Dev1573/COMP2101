...
#We will use some command to get some output that can be send to stderr
	dn=$(hostname --fqdn)
echo "${dn}: Error! Please Try Again" >&2
}

echo "$error-message The output of command is thrown to stderr"
# This function will send a message to stderr and exit with a failure status
function error-exit {
	error-message "$1"
#By using following command we can get exit status
	exit "${2:-1}"
}
...
#We will make new function failure to be more accurate for user
function failure {
echo "$@" >&2
}

#Now it's time to use case command

while [ $# -gt 0 ]: do
	case "$1" in

#Following script will show output if user write -h in command
		-h|--help)
		displayhelp
		exit
		;;

#Next command line will give output if user write a wrong command line
		*)
		failure "Your Command line is wrong. Try again."
		failure "$(displayhelp)"
		exit
		;;
