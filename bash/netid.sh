...
#First we will use while loop make and run our script according to lesson
while [ $# -gt 0 ]; do
[ "$verbose" = "yes" ] && echo we used $# to find the command line
[ "$verbose" = "yes" ] && echo "Next we will use $1 on the command line"

#Second we will use case command in the command line
case "$1" in
...
#Third we will use case command to find out is there any string in the command line
# And also let's use if command in the following command line

	[0-9])
		if [ "$verbose" = "yes" ]; then
			echo 'Oops! You have used -v'
			exit
		else
			echo $(netid.sh)
			echo 'You have used string in command line, so output is for the interface'
			exit
		fi
		;;

#We will use this same mathod for Capital letters
...
#If user is looking for a list of interface we will made another script for it
#We will choose LIST word to get a list of ip interface

	LIST )
		if [ "$verbose" = "yes" ]; then
			echo 'Command line is processed'
			exit
#By using else command we can joint for loop to get a list of ip interface
		else
			for d in $(ifconfig | awk '/: /{print $1}'); do
			echo "$d"
		done
		fi
		;;
